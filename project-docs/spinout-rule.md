...
- **The pointer paragraph the parent keeps will state facts and cite nothing.** Three cases so far, all found later by the uncited-entries check rather than at the time: the Hay Finlay paragraph in `canadian-camping-movement.md`, the Voyageurs paragraph in `canoe-trips.md`, and the Indian-lore paragraph in `indigenous-names-and-land.md`. The summary sentence gets written fresh, from memory of what was just moved, and the markers stay behind with the paragraphs. Whatever the pointer asserts, mark it — usually with the same entries the cut just orphaned, which fixes both faults at once.
- **An article part-way between a lettered and a numbered scheme can carry a marker that resolves to nothing.** `canoe-trips.md` used `^mc` five times for its central chronology and had no entry `mc` at all; the citation checks resolve numeric markers and ignore lettered ones, so nothing saw it (f_5730). The uncited-entries report is the tell: the entry that marker *should* have pointed at shows up unpointed-at.
